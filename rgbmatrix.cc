...
static PyObject *SetBuffer(RGBmatrixObject *self, PyObject *data) 
{
    ...
    for(y=0; y<h; y++) 
    {
        ...
        {
            offset = (y*w*3)+(x*3);
            r = PyLong_AsLong(PyList_GetItem(data, offset));
            g = PyLong_AsLong(PyList_GetItem(data, offset+1));
            b = PyLong_AsLong(PyList_GetItem(data, offset+2));
...
PyMODINIT_FUNC PyInit_rgbmatrix(void) { // Module initialization function
        PyObject* m;

    static struct PyModuleDef moduledef = {
        PyModuleDef_HEAD_INIT,
        "rgbmatrix",                 /* m_name */
        "Adafruit RGBMatix Module",  /* m_doc */
        -1,                          /* m_size */
        methods,                     /* m_methods */
        NULL,                        /* m_reload */
        NULL,                        /* m_traverse */
